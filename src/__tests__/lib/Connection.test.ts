...
import neo4jDriver, { Session, auth } from 'neo4j-driver';
...
describe('Connection', () => {
    ...
    it('should establish a connection when constructed', () => {
        spyOn(auth, 'basic').mockResolvedValueOnce(null as unknown as never);
    ...
    it('should get a session', () => {
        ...
        expect(session).toStrictEqual(mockSession as unknown as Session);
    });
